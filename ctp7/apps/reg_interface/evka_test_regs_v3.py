    ...
    for i in range(0, numIterations):
        ...
        if value != initValue & regMask:
            if value == 0xdeaddead & regMask:
                busErrors += 1
                printRed("Bus error.. :/ got this value here: " + hex(value) + " (expected " + hex(initValue & regMask) + ")")
            else:
                valueErrors += 1
...
def checkVfat3SlowControlStatus():
    print("----==== VFAT3 slow control status ====----")
    print("    CRC error count:         %d" % parseInt(readReg(getNode('GEM_AMC.SLOW_CONTROL.VFAT3.CRC_ERROR_CNT'))))
    print("    Packet error count:      %d" % parseInt(readReg(getNode('GEM_AMC.SLOW_CONTROL.VFAT3.PACKET_ERROR_CNT'))))
