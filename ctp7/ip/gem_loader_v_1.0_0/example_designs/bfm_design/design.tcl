proc create_ipi_design { offsetfile design_name } {
	...
	# Create instance: gem_loader_v_0, and set properties
	set gem_loader_v_0 [ create_bd_cell -type ip -vlnv user.org:user:gem_loader_v:1.0 gem_loader_v_0]
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI] [get_bd_intf_pins gem_loader_v_0/S_C2C_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/M_AXI_ACLK] [get_bd_pins gem_loader_v_0/S_C2C_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/M_AXI_ARESETN] [get_bd_pins gem_loader_v_0/S_C2C_AXI_ARESETN]
	...
	# Create interface connections
	connect_bd_intf_net [get_bd_intf_pins master_1/M_AXI_LITE] [get_bd_intf_pins gem_loader_v_0/S_CFG_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_1/M_AXI_LITE_ACLK] [get_bd_pins gem_loader_v_0/S_CFG_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_1/M_AXI_LITE_ARESETN] [get_bd_pins gem_loader_v_0/S_CFG_AXI_ARESETN]
	...
	set offset_file "${bd_path}/gem_loader_v_v1_0_tb_include.vh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef gem_loader_v_v1_0_tb_include_vh_"
	puts $fp "`define gem_loader_v_v1_0_tb_include_vh_\n"
	...
	puts $fp "`define S_C2C_AXI_SLAVE_ADDRESS ${offset_hex}"

	set offset [get_property OFFSET [get_bd_addr_segs -of_objects [get_bd_addr_spaces master_1/Data_lite]]]
	set offset_hex [string replace $offset 0 1 "32'h"]
	puts $fp "`define S_CFG_AXI_SLAVE_ADDRESS ${offset_hex}"

	puts $fp "`endif"
	close $fp
}

set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:gem_loader_v:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/gem_loader_v_v1_0_tb.v
...
for { set i 1 } { 1 } { incr i } {
	set design_name "gem_loader_v_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 gem_loader_v_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top gem_loader_v_v1_0_tb [get_filesets sim_1]
